...
SELECT country, COUNT(*) AS suppliers_count FROM suppliers GROUP BY country ORDER BY suppliers_count DESC;
...
SELECT ship_country, SUM(freight) FROM orders WHERE ship_region IS NOT NULL GROUP BY ship_country
HAVING SUM(freight) > 2750 ORDER BY SUM(freight) DESC;

-- 6. страны, в которых зарегистрированы и заказчики (customers) и поставщики (suppliers) и работники (employees).
SELECT DISTINCT country FROM customers INTERSECT
SELECT DISTINCT country FROM employees INTERSECT
...
SELECT DISTINCT country FROM customers INTERSECT
SELECT DISTINCT country FROM suppliers EXCEPT
